...
{c.logo_url && (
  ...
  if ((section.section_key === "market_study_subject" || section.section_key === "market_study_placeholder") && c.status === "completed") {
    const sp = section.section_key === "market_study_subject" ? c.subject_property : c.subject_property;
    if (!sp && section.section_key === "market_study_subject") {
      return (
        <div className="w-full h-full bg-white p-16" style={{ fontFamily: FONT }}>
          <SlideLabel color={accent}>Análise de mercado</SlideLabel>
          <h2 className="slide-title mt-4" style={{ fontSize: theme.heading.titleSize, color: primary, textTransform }}>Imóvel Avaliado</h2>
    ...
    return (
      <div className="w-full h-full bg-white p-16" style={{ fontFamily: FONT }}>
        <SlideLabel color={accent}>Análise de mercado</SlideLabel>
        <h2 className="slide-title mt-4 mb-8" style={{ fontSize: theme.heading.titleSize, color: primary, textTransform }}>Imóvel Avaliado</h2>
        <SlideDivider theme={theme} colors={colors} />
        {sp && (
          <div className="mt-8 space-y-8">
            ...
            {sp.differentials?.length > 0 && (
              <div className="mt-4">
                <p className="font-bold mb-3" style={{ fontSize: "22px", color: primary }}>Diferenciais</p>
                <div className="flex flex-wrap gap-3">
                  {sp.differentials.map((d: any, i: number) => (
                    <span key={i} className="px-5 py-2 rounded-full font-medium" style={{ fontSize: "20px", backgroundColor: accent + "15", color: accent }}>
            ...
            {sp.owner_expected_price && (
              <p className="mt-4" style={{ fontSize: "26px", color: textMuted }}>
                Valor pretendido: <span className="font-bold" style={{ color: primary }}>R$ {Number(sp.owner_expected_price).toLocaleString("pt-BR")}</span>
              </p>
            )}
            {c.executive_summary && (
              <p className="mt-4 leading-relaxed" style={{ fontSize: "22px", color: textMuted }}>{c.executive_summary}</p>
            )}
          </div>
        )}
      </div>
