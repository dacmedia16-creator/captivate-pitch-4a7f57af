...
export default function AgentNewPresentation() {
  ...
  const runMarketAnalysisBackground = async (
    ...
    const { data: job, error: jobError } = await supabase.from("market_analysis_jobs").insert({
      ...
      tenant_id: tenantId,
      selected_portals: mktData.selectedPortals as any,
      filters: {
        ...
        preferSameCondominium: mktData.preferSameCondominium,
      } as any,
      status: "pending",
      started_at: new Date().toISOString(),
    }).select().single();

    if (!job || jobError) return;
    ...
    if (generatedComparables.length > 0) {
      marketCalc = calculateMarketPrices(generatedComparables.map(c => ({
        price: c.price, price_per_sqm: c.price_per_sqm, is_approved: c.is_approved ?? true,
      })));
      ...
      const [, reportRes] = await Promise.all([
        ...
        supabase.from("market_reports").insert({
          market_analysis_job_id: job.id, avg_price: marketCalc.avg_price, median_price: marketCalc.median_price,
          avg_price_per_sqm: marketCalc.avg_price_per_sqm, suggested_market_price: marketCalc.suggested_market_price,
          ...
          confidence_level: "medium", summary: summaryText,
        }).select().single(),
        supabase.from("market_analysis_jobs").update({ status: "completed", finished_at: new Date().toISOString() }).eq("id", job.id),
      ]);

      marketReport = reportRes?.data;
    } else {
      await supabase.from("market_analysis_jobs").update({ status: "failed", finished_at: new Date().toISOString() }).eq("id", job.id);
    }
    ...
    try {
      const { data: study } = await supabase.from("market_studies").insert({
        ...
        title: propData.title || "Estudo de Mercado", purpose: propData.property_purpose || "venda", status: "completed",
      }).select().single();
      ...
      if (study) {
        ...
        if (scrapedComparables.length > 0) {
          ...
          const studyComparables = scored.map((c: any) => ({
            ...
            source_name: c.source_name || null, source_url: c.source_url || null, is_approved: true,
          }));
          ...
          if (insertedComps && insertedComps.length > 0) {
            ...
            try {
              const { data: aiSummary } = await supabase.functions.invoke("generate-market-summary", {
                body: { subject: subjectForScoring, comparables: studyComparables, result: { ...result, avg_price_per_sqm: Math.round(avgPriceSqm) } },
              });
    ...
    // Update presentation sections with market data
      ...
      await Promise.all([
        ...
        supabase.from("presentation_sections").update({
          content: {
            ...
            owner_expected_price: propData.owner_expected_price ? Number(propData.owner_expected_price) : null,
            avg_price: marketCalc.avg_price, median_price: marketCalc.median_price,
            avg_price_per_sqm: marketCalc.avg_price_per_sqm, status: "completed",
          } as any,
