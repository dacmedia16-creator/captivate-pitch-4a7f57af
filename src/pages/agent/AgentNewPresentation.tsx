        ...
        if (marketData.selectedPortals.length > 0) {
          const { data: job, error: jobError } = await supabase.from("market_analysis_jobs").insert({
            ...
            filters: {
              ...
              preferSameCondominium: marketData.preferSameCondominium,
            } as any,
            status: "pending",
          ...
          if (job && !jobError) {
            ...
            const analyzeBody = {
              ...
              filters: {
                ...
                maxComparables: marketData.maxComparables,
                preferSameCondominium: marketData.preferSameCondominium,
                maxListingAgeMonths: marketData.maxListingAgeMonths,
              },
            };
            ...
            const subjectForScoring = {
              ...
              city: propertyData.city || null,
              property_type: propertyData.property_type || null,
              area_useful: propertyData.area_useful ? Number(propertyData.area_useful) : (propertyData.area_total ? Number(propertyData.area_total) : null),
              area_built: propertyData.area_built ? Number(propertyData.area_built) : null,
              area_land: propertyData.area_land ? Number(propertyData.area_land) : null,
              bedrooms: propertyData.bedrooms ? Number(propertyData.bedrooms) : null,
              suites: propertyData.suites ? Number(propertyData.suites) : null,
              parking_spots: propertyData.parking_spots ? Number(propertyData.parking_spots) : null,
              construction_standard: propertyData.construction_standard || propertyData.property_standard || null,
              conservation_state: propertyData.conservation_state || null,
              owner_expected_price: propertyData.owner_expected_price ? Number(propertyData.owner_expected_price) : null,
            };
            ...
            try {
              ...
              if (study) {
                ...
                await supabase.from("market_study_subject_properties").insert({
                  ...
                  bathrooms: propertyData.bathrooms ? Number(propertyData.bathrooms) : null,
                  parking_spots: propertyData.parking_spots ? Number(propertyData.parking_spots) : null,
                  living_rooms: propertyData.living_rooms ? Number(propertyData.living_rooms) : null,
                  powder_rooms: propertyData.powder_rooms ? Number(propertyData.powder_rooms) : null,
                  construction_standard: propertyData.construction_standard || propertyData.property_standard || null,
                  conservation_state: propertyData.conservation_state || null,
                  differentials: propertyData.differentials.length > 0 ? propertyData.differentials as any : [],
                  condominium_fee: propertyData.condominium_fee ? Number(propertyData.condominium_fee) : null,
                  iptu: propertyData.iptu ? Number(propertyData.iptu) : null,
                  pricing_objective: propertyData.pricing_objective || null,
                  property_age: propertyData.property_age || null,
                  owner_expected_price: propertyData.owner_expected_price ? Number(propertyData.owner_expected_price) : null,
                  observations: propertyData.notes || null,
                  purpose: propertyData.property_purpose || "venda",
                });
